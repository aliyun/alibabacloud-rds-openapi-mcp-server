...
钉钉接入使用 [钉钉开放平台](https://open.dingtalk.com/) 应用机器人和 Stream 模式。

### 开放平台配置

1. 创建应用：登录钉钉开放平台，在右上角单击开发者后台。进入后台后，在应用开发 > 钉钉应用中单击创建应用，依次填写应用名称、应用描述和应用图标，单击确定创建。
2. 添加并配置机器人：应用创建完成后，单击添加机器人。在机器人配置页面填写机器人名称、机器人图标、简介和功能描述；消息接收模式选择 Stream 模式，然后单击发布。
3. 配置权限：在左侧导航栏单击权限管理，申请并开通以下权限：
   - 企业内机器人发送消息权限
   - 互动卡片实例写权限
   - AI 卡片流式更新权限
4. 发布应用版本：在左侧导航栏单击版本管理与发布，单击创建新版本。填写版本描述，按需选择应用可用范围，然后单击直接发布。发布完成后，确认版本状态为已上线。
5. 获取应用凭证：在应用基础信息中复制 Client ID 和 Client Secret，分别写入 `.env` 的 `DINGTALK_APP_CLIENT_ID` 和 `DINGTALK_APP_CLIENT_SECRET`。
...
飞书接入使用 [飞书开放平台](https://open.feishu.cn/) 企业自建应用机器人和长连接模式。

### 开放平台配置

1. 创建应用：登录飞书开放平台，在右上角单击开发者后台。进入后台后单击创建企业自建应用，填写应用名称、应用描述和应用图标，单击创建。
2. 批量配置机器人权限：在左侧导航栏单击权限管理，进入权限配置页后单击批量导入/导出权限。将下面的 JSON 权限描述复制到导入页签，单击下一步，确认新增权限后单击申请开通。
...
3. 配置加密策略：单击左侧导航栏事件与回调，进入加密策略页签。单击 Encrypt Key 区域的刷新按钮重置 Encrypt Key，并妥善保存；当前长连接模式不需要在 `.env` 中配置该值。
4. 启用事件订阅：在事件与回调中订阅 `im.message.receive_v1` 事件，用于接收单聊和群聊消息。确认应用机器人能力已启用。
5. 获取应用凭证：在凭证与基础信息中复制 App ID 和 App Secret，分别写入 `.env` 的 `FEISHU_APP_ID` 和 `FEISHU_APP_SECRET`。
